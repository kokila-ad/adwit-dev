...
class Login extends Management_Controller {
	...
	public function index($error = '', $page = 1, $color = 'darkred')
	{
		...
		if(!$this->session->userdata('management'))
			$this->load->view('management/login',$data);
		else
	...
	{
		return $this->db->query("Select * from management where (username='".$username."' OR email_id='".$username."') and password='".md5($password)."' and is_active='1' and is_deleted='0'")->num_rows();
	}
	...
	public function change($encrypted_key)
	{
		...
		if($secrets[1] + 172800 > mktime() && $this->secretExists($secrets[0],$encrypted_key)==1)
		{
			...
			$this->load->view('management/reset',$data);
		}else
		{
	...
	{
		return $this->db->query("Select * from management where email_id='".$email_id."' and encrypted_key='".$encrypted_key."' and is_active='1' and is_deleted='0'")->num_rows();
	}
	...
	public function dochange($encrypted_key)
	{
		...
		if($secrets[1] + 172800 > mktime() && $this->secretExists($secrets[0],$encrypted_key)==1)
		{
			$this->db->update('management',array('encrypted_key' => '', 'password' => md5($this->input->post('new_password'))),array('email_id' => $secrets[0], 'encrypted_key' => $encrypted_key));
