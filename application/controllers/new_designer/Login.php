...
class Login extends CI_Controller {
	...
	public function index($error = '', $page = 1, $color = 'darkred')
	{
		...
		if(!$this->session->userdata('designer'))
			redirect('client/login'); //$this->load->view('new_designer/login',$data);
		else
	...
	public function doIt()
	{
		...
		if($this->logExists($this->input->post('username'), $this->input->post('password'))==1)
		{
			...
			{
				$this->session->sess_expiration = 8200;
			}
			
			$result = $this->db->get_where('designers', array('username' => $this->input->post('username')))->result_array();
			$userdata = array('designer' => $this->input->post('username'), 'dId' => $result[0]['id'], 'dEmail' => $result[0]['email_id'], 'dLoc' => $result[0]['Join_location'], 'dTeamId' => $result[0]['adwit_teams_id']);
			...
			$this->user_login_session($this->session->userdata('dId'), 4, 'in');
			$this->index();
	...
	{
		return $this->db->query("Select * from designers where (username='".$username."' OR email_id='".$username."') and password='".md5($password)."' and is_active='1' and is_deleted='0'")->num_rows();
	}
	...
	public function reset()
	{
		...
		if($this->emailIdExists($this->input->post('email_id')))
		{
			...
			$data['recipient_display'] = 'Designers Adwit Ads';
			$data['headline'] = 'Hello!';
	...
	public function change($encrypted_key)
	{
		...
		if($secrets[1] + 172800 > time() && $this->secretExists($secrets[0],$encrypted_key)==1){
			...
			$this->load->view('new_designer/reset',$data);
		}else{
	...
	{
		return $this->db->query("Select * from designers where email_id='".$email_id."' and encrypted_key='".$encrypted_key."' and is_active='1' and is_deleted='0'")->num_rows();
	}
	...
	public function dochange($encrypted_key)
	{
		...
		if($secrets[1] + 172800 > time() && $this->secretExists($secrets[0],$encrypted_key)==1)
		{
			$this->db->update('designers',array('encrypted_key' => '', 'password' => md5($this->input->post('new_password')), 'pwd_date' => date('Y-m-d')),array('email_id' => $secrets[0], 'encrypted_key' => $encrypted_key));
	...
	{
		$this->user_login_session($this->session->userdata('dId'), 4, 'out');
	    $this->db->close();
