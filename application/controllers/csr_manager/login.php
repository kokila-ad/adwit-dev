...
class Login extends Cmanager_Controller {
	...
	public function index($error = '', $page = 1, $color = 'darkred')
	{
		...
		if(!$this->session->userdata('csr_manager'))
			$this->load->view('csr_manager/login',$data);
		else
	...
	public function doIt()
	{
		...
		if($this->logExists($this->input->post('username'), $this->input->post('password'))==1)
		{
			...
			$result = $this->db->get_where('csr_manager', array('username' => $this->input->post('username')))->result_array();
			$userdata = array('csr_manager' => $this->input->post('username'), 'cmId' => $result[0]['id'], 'cmEmail' => $result[0]['email_id']);
	...
	{
		return $this->db->query("Select * from csr_manager where (username='".$username."' OR email_id='".$username."') and password='".md5($password)."' and active='1'")->num_rows();
	}
	...
	public function reset()
	{
		if($this->emailIdExists($this->input->post('email_id')))
		{
			...
			$this->db->update('csr_manager',array('encrypted_key' => $secret_key), array('email_id' => $this->input->post('email_id')));
			...
			$data['p_three'] = 'Click Here to <a href="'.base_url().index_page().'csr_manager/login/change/'.$secret_key.'" style="text-decoration:none;color:#fcb400;">Reset Your Password</a>';
	...
	{
		return $this->db->query("Select * from csr_manager where email_id='".$email_id."' and active='1'")->num_rows();
	}
	...
	public function change($encrypted_key)
	{
		...
		if($secrets[1] + 172800 > mktime() && $this->secretExists($secrets[0],$encrypted_key)==1)
		{
			...
			$this->load->view('csr_manager/reset',$data);
		}else
		{
	...
	{
		return $this->db->query("Select * from csr_manager where email_id='".$email_id."' and encrypted_key='".$encrypted_key."' and active='1'")->num_rows();
	}
	...
	public function dochange($encrypted_key)
	{
		...
		if($secrets[1] + 172800 > mktime() && $this->secretExists($secrets[0],$encrypted_key)==1)
		{
			$this->db->update('csr_manager',array('encrypted_key' => '', 'password' => md5($this->input->post('new_password'))),array('email_id' => $secrets[0], 'encrypted_key' => $encrypted_key));
