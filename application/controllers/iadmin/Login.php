...
class Login extends Iadmin_Controller {
	...
	public function index($error = '', $page = 1, $color = 'darkred')
	{
		...
		if(!$this->session->userdata('iadmin'))
			$this->load->view('iadmin/login',$data);
		/*else
	...
	public function reset()
	{
		if($this->emailIdExists($this->input->post('email_id')))
		{
			...
			$this->db->update('iadmin_users',array('encrypted_key' => $secret_key), array('email_id' => $this->input->post('email_id')));
			...
			$data['p_three'] = 'Click Here to <a href="'.base_url().index_page().'iadmin/login/change/'.$secret_key.'" style="text-decoration:none;color:#fcb400;">Reset Your Password</a>';
	...
	{
		return $this->db->query("Select * from iadmin_users where email_id='".$email_id."' and is_active='1' and is_deleted='0'")->num_rows();
	}
	...
	public function change($encrypted_key)
	{
		...
		if($secrets[1] + 172800 > mktime() && $this->secretExists($secrets[0],$encrypted_key)==1)
		{
			...
			$this->load->view('iadmin/reset',$data);
		}else
		{
	...
	{
		return $this->db->query("Select * from iadmin_users where email_id='".$email_id."' and encrypted_key='".$encrypted_key."' and is_active='1' and is_deleted='0'")->num_rows();
	}
	...
	public function dochange($encrypted_key)
	{
		...
		if($secrets[1] + 172800 > mktime() && $this->secretExists($secrets[0],$encrypted_key)==1)
		{
			$this->db->update('iadmin_users',array('encrypted_key' => '', 'password' => md5($this->input->post('new_password'))),array('email_id' => $secrets[0], 'encrypted_key' => $encrypted_key));
