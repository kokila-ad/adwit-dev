...
class Login extends Hr_Controller {
	...
	public function doIt()
	{
		...
		if($this->logExists($this->input->post('username'), $this->input->post('password'))==1)
		{
			...
			$result = $this->db->get_where('hr', array('username' => $this->input->post('username')))->result_array();
			$userdata = array('hr' => $this->input->post('username'), 'hrId' => $result[0]['id'], 'hrEmail' => $result[0]['email_id'], 'hrName' => $result[0]['name']);
	...
	{
		return $this->db->query("Select * from hr where (username='".$username."') and password='".md5($password)."' and is_active='1' and is_deleted='0'")->num_rows();
	}
	...
	public function reset()
	{
		if($this->emailIdExists($this->input->post('email_id')))
		{
			...
			$this->db->update('hr',array('encrypted_key' => $secret_key), array('email_id' => $this->input->post('email_id')));
	...
	{
		return $this->db->query("Select * from hr where email_id='".$email_id."' and is_active='1' and is_deleted='0'")->num_rows();
	}
	...
	{
		return $this->db->query("Select * from hr where email_id='".$email_id."' and encrypted_key='".$encrypted_key."' and is_active='1' and is_deleted='0'")->num_rows();
	}
	...
	public function dochange($encrypted_key)
	{
		...
		if($secrets[1] + 172800 > mktime() && $this->secretExists($secrets[0],$encrypted_key)==1)
		{
			$this->db->update('hr',array('encrypted_key' => '', 'password' => md5($this->input->post('new_password'))),array('email_id' => $secrets[0], 'encrypted_key' => $encrypted_key));
