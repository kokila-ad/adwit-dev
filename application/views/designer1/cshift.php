...
<?php 
	foreach($publication as $row)
	{
			...
			{
				$orders = $this->db->query("SELECT * FROM `orders` WHERE `publication_id`='".$row['id']."' AND (`status`='2' || `status`='3' || `status`='4') AND `cancel`='0' AND `crequest`='0' AND (`created_on` BETWEEN '$from' AND '$to') ;")->result_array();
			}else{
				$orders = $this->db->query("SELECT * FROM `orders` WHERE `publication_id`='".$row['id']."' AND (`status`='2' || `status`='3' || `status`='4') AND `cancel`='0' AND `crequest`='0' AND (`created_on` BETWEEN '$ystday' AND '$today') ;")->result_array();		
