  ...
  <div class="row-fluid" style="width:96%; margin: 0 auto;">  
    ...
    <div class="block">
      <div class="navbar navbar-inner block-header">
		...
		<div class="muted pull-left">Designer Report :<?php echo $year; ?></div>
      ...
      <div class="block-content collapse in">
        <div class="span12">
          <table cellpadding="0" cellspacing="0" border="0" class="table table-striped table-bordered" id="example">
			...
			<thead>
            <tr>
				<th rowspan="2" style="vertical-align: middle;">Dname</th>
				<th rowspan="2" style="vertical-align: middle;">Dcode</th>
				<th rowspan="2" style="vertical-align: middle;">Location</th>
				<?php for($i = 1 ; $i <= 12; $i++){ ?>
				<th rowspan="2" style="vertical-align: middle;"><?php echo date("M",strtotime(date("Y")."-".$i."-01")); ?></th>
				<?php } ?>
			</tr>
...
<?php
	foreach($designers as $row){ 
		$dId = $row['id'];
		$y = $year;
		$location = $this->db->get_where('location',array('id'=>$row['Work_location']))->result_array();
		for($i = 1 ; $i <= 12; $i++){
			$d = date('Y-m', strtotime("$i month", strtotime("$y-00")));
			$cat_result[$i] = $this->db->query("SELECT * FROM `cat_result` WHERE `designer`='$dId' AND `ddate` LIKE '$d%' ")->num_rows();
		}
...
?>
              <tr class="odd gradeX">
                ...
                <td><?php if($row['Work_location']!='0') echo $location[0]['name']; ?></td>
				<?php for($i = 1 ; $i <= 12; $i++){  ?>
                <td><?php echo $cat_result[$i]; ?></td>
				<?php } ?>
			  </tr>
<?php }  ?>
            </tbody>
