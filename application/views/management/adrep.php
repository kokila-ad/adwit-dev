					...
					<div class="portlet light">
						<div class="portlet-title">
							<div class="row static-info">
								<div class="col-md-8 value margin-top-10">Advertiser Representative</div>
						...
						<div class="portlet-body">
							...
							<div class="col-md-12">
								<table class="table table-striped table-bordered table-hover" id="sample_2">
										...
										$pub_name = $this->db->query("SELECT * FROM `publications` WHERE `id` = '".$row['publication_id']."' ")->result_array();
										$group_name = $this->db->query("SELECT * FROM `Group` WHERE `id` = '".$pub_name[0]['group_id']."' ")->result_array();
										$orders = $this->db->query("SELECT * FROM `orders` WHERE `adrep_id` = '".$row['id']."'  AND `pdf`!='none' AND (`created_on` BETWEEN '$past_month' AND '$cur_date')")->result_array();
										...
										<tr>
											...
											<td><?php if($row['publication_id'] == '0'){ echo 'none'; }else{ echo $pub_name[0]['name']; } ?></td>
											<td><?php if($pub_name[0]['group_id'] == '0'){echo 'none';}else{ echo $group_name[0]['name'];}?></td>
											<td><?php if($orders){
													$count = count($orders);
													$avg = $count/6;
													echo (round($avg,2));
