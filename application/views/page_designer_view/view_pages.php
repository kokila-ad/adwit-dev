...
?>
<?php $this->load->view('new_designer/header') ?>
<style>
.right{
	float: right;
}
									...
									<div class="row">
										<div class="col-md-8 col-sm-8 col-xs-8 padding-right-0">
							 			...
							 			<div class="col-md-4 col-sm-4 col-xs-4 padding-left-0">
											<a href="<?php echo base_url().index_page()."new_designer/home/index/"?>" class="text-blue right"><i class="fa fa fa-angle-double-left"></i> back</a>
										</div>
						 			</div>	
												...
												<td>
        											...
        											if ($articles_path != null && file_exists($articles_path) && $atp = opendir($articles_path)) //check the notnull exitsfile and openfile
        											{
            											while (($file = readdir($atp)) !== false)  //loop to read the file path then store it
            											{
                											...
                											{?>
                												<?php echo "<b>".$i."."; ?>
                												<a href='<?php echo base_url().'page_design/articles'.'/'.$key['pd_id'].'/'.$key['id'].'/'.$file?>' target="_blank"><?php echo $file;?><br></a>
                											<?php }
...
<?php $this->load->view('new_designer/footer') ?>
