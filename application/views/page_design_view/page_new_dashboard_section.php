        ...
        <div class="container margin-top-50"> 
			...
			<div class="row">
				<div class="col-md-9 padding-15">
				    <div class="btn-group w-100 " data-kt-buttons="true" data-kt-buttons-target="[data-kt-button]"></div>
				</div>
    		    ...
    		    <div class="col-md-12  alltable">
    			    <div class="table-responsive border padding-15">     
    					<table class="kt_datatable_example_5 table table-striped table-bordered table-hover table-row-bordered gy-5 gs-7 border rounded " >
    						<thead>
    							<tr class="fw-bolder fs-6 text-gray-800 px-7 bold">
    							    <td></td>
...
//dashboard table content    
    function LoadTableContent(status, from_date, to_date){ //alert(status);
        ...
        var dataTable = $('.kt_datatable_example_5').DataTable({
            ...
            "order": [],  
            "ajax": {  
                url:"<?php echo base_url().index_page().'new_client/home/page_dashboard_content_section'; ?>",  
                type:"GET",
    ...
    function LoadOrders(row, pd_id){ 
        
        if(row.hasClass("fa-chevron-circle-down")){
            row.closest('tr').after('<tr><tr id="row1" class="loading"> <td></td> <td colspan="5">Loading...</td> </tr></tr>');
            $.get("<?php echo base_url().index_page().'new_client/home/page_orders_section/'; ?>"+pd_id, function(data){
                removeElementsByClass('loading');   
                row.closest('tr').after('<tr>'+data+'</tr>');
            });
        }else{
            row.siblings('tr').remove();
            removeElementsByClass('sub-table');
        }
        
         row.toggleClass("fa-chevron-circle-down fa-chevron-circle-up");
        
    }
   //remove elements of classname 
