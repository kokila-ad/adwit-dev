	...
	<div class="portlet-title margin-bottom-25">
		<div class="row margin-bottom-5">	
			<div class="col-md-8 col-xs-12">
				...
				<a href="<?php echo base_url().index_page().'new_admin/home/designer_details'?>" class="font-lg font-grey-gallery"><u>Designer</u></a>
			</div>							
	...
	<div class="portlet-body">
		<div class="row">
		 <div class="col-md-6 margin-bottom-20">
			<p class="bold align-center" >Designer - 
			<a href="<?php echo base_url().index_page().'new_admin/home/designer'; ?>"><small class="btn btn-xs bg-grey btn-circle"><?php echo count($designer); ?></small></a></p>
...
<script>
		var chart;
		var chartData = [   <?php foreach($designer_role as $role){ 
									$count = $this->db->query("SELECT * FROM `designers` WHERE `designer_role` = '".$role['id']."' AND `is_active` = '1'")->num_rows();
							?>
							{ "adrep": "<?php echo $role['name'] ?>","designers": <?php echo $count;?> },
							<?php } ?>
						];
		...
		AmCharts.ready(function () {
			...
			graph1.type = "column";
			graph1.valueField = "designers";
			graph1.balloonText = "Designers:[[value]]";
			graph1.lineAlpha = 0;
...
<script>
		...
		<?php foreach($location as $loc){ 
					$loc_count = $this->db->query("SELECT * FROM `designers` WHERE `Join_location` = '".$loc['id']."' AND `is_active` = '1'")->num_rows();
			?>
			{  "adrep": "<?php echo $loc['name'] ?>",	"designers": <?php echo $loc_count;?> },
			<?php } ?>
		];
