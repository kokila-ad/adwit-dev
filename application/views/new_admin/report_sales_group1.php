...
<div class="portlet light">
	...
	foreach($groups as $row){
		...
		$publication = $this->db->query("SELECT * FROM `publications` WHERE `group_id` = '".$row['id']."' AND `is_active` = '1'")->result_array();
		if($order_type == '2'){
			$orders = $this->db->query("SELECT * FROM `orders` WHERE `order_type_id` = '2' AND `group_id` = '".$row['id']."' AND (`created_on` BETWEEN '$from 00:00:00' AND '$to 23:59:59') AND (`pdf` != 'none')")->result_array();
		}elseif($order_type == '1'){
			$orders = $this->db->query("SELECT * FROM `orders` WHERE `order_type_id` = '1' AND `group_id` = '".$row['id']."' AND (`created_on` BETWEEN '$from 00:00:00' AND '$to 23:59:59') AND (`pdf` != 'none')")->result_array();
		}else{
		$orders = $this->db->query("SELECT * FROM `orders` WHERE `group_id` = '".$row['id']."' AND (`created_on` BETWEEN '$from 00:00:00' AND '$to 23:59:59') AND (`pdf` != 'none')")->result_array();
		}
	...
	<div class="portlet-body">
		<div class="row report margin-0 border-top border-bottom">
			<div class="col-md-3 col-sm-6 report-tab">
				<h3 class="font-blue"><?php echo $groups[0]['name'] ;?></h3>
			...
			<div class="col-md-3 col-sm-6 report-tab padding-left-40">
			<a href="javascript:;" onclick="window.location ='<?php echo base_url().index_page().'new_admin/home/group_publication_details/'.$row['id'].'/'.$type.'/'.$user.'/'.$order_type.'/'.$date.'?from='.$from.'&to='.$to;?>'" style="cursor:pointer; text-decoration: none;">
				<h3 class="font-grey-gallery"><?php echo count($publication);?></h3></a>
	...
	<a href="javascript:;" onclick="window.location ='<?php echo base_url().index_page().'new_admin/home/grp_category_wise/'.$row['id'].'/'.$type.'/'.$user.'/'.$date.'?from='.$from.'&to='.$to;?>'" style="cursor:pointer; text-decoration: none;">
