...
<div id="Middle-Div">
  ...
  <?php echo '<h5 style="color:#900;">'.$this->session->flashdata('message').'</h5>'; ?> 
    ...
    <div class="block">
      <div class="navbar navbar-inner block-header">
        <div class="muted pull-left">Softwrite Orders  <?php //if(isset($from) && isset($to)){echo $from." to ".$to;} ?></div>
 ...
 <?php 
	foreach($softwrite_orders as $row){ 
		$users = $this->db->get_where('soft_users',array('id' => $row['user']))->result_array();
		$pub = $this->db->get_where('soft_publications',array('id' => $row['publication']))->result_array();
?>    
              <tr class="odd gradeX">
                <td><?php echo $row['timestamp']; ?></td>
				<td><?php echo 'S'.$row['id']; ?></td>
				<td><?php echo $row['ad_identifier']; ?></td>
				<td><?php echo $users[0]['first_name']; ?></td> 
				<td><?php echo $pub[0]['name']; ?></td> 
