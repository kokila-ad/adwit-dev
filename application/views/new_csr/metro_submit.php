													...
													$adreps = $this->db->query("SELECT  adreps.id, adreps.first_name, adreps.publication_id
																		FROM adreps 
																			left outer join publications on adreps.publication_id = publications.id 
																				where publications.channel = '1' ORDER BY adreps.first_name ASC;")->result_array();
												?>
									...
									<div class="row margin-top-15">	
										...
										<div class="col-md-6">
												...
												if($metro_file_map){ foreach($metro_file_map as $row){ 
													...
													if(is_dir($row)){ 
														...
														foreach($map2 as $row2){
															$dir2 = dirname($row2);	
															$dir2 = explode('/', $dir2); //echo end($dir2)."<br/>";
															if(end($dir2) != 'THUMBS'){
											?>
											<input type="text" name="attachfile[]" value="<?php echo $row2; ?>" style="display:none"/>
												...
												<a href="<?php echo 'https://adwitads.com/metroaod/incoming/New Order/'.end($dir).'/'.end($dir2).'/'.basename($row2); ?>"  target="_Blank" class="btn btn-circle btn-xs"><span class="fa fa-eye"></span></a>
											...
											<input type="text" name="attachfile[]" value="<?php echo $row; ?>" style="display:none"/>
												...
												<a href="<?php echo 'https://adwitads.com/metroaod/incoming/New Order/'.end($dir).'/'.basename($row); ?>"  target="_Blank" class="btn btn-circle btn-xs"><span class="fa fa-eye"></span></a>
