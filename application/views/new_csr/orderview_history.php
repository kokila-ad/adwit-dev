			...
			<div class="portlet-titl no-space margin-top-10">
				<div class="row static-info">
					<div class="col-sm-6 value ">
					    AdwitAds ID: <?php echo $order_id; ?><?php foreach($orders as $row)?><small class="font-grey-cascade"> &nbsp; 
					(<?php $date = strtotime($row['created_on']); echo date('d F', $date).','.date('Y', $date).' '.'at'.' '.date('g:i A', $date);?>)</small>
					</div>
					<div class="col-sm-6 text-right">
						<div class="tools">
							<button class="btn bg-blue btn-xs no-margin" id="show_subrev"> Submit Revision</button>	
		...
		<div class="portlet light margin-0">
			<div class="portlet-titl no-space margin-top-10 margin-bottom-10">
				<div class="row static-info">
					...
					<div class="col-sm-6 text-right">
						<div class="tools">
						<?php if($rev_orders[0]['status'] == '5') { ?>
						<button class="btn bg-blue btn-xs no-margin" id="show_subrev"> Submit Revision</button>	<?php } ?>
							<button onclick="goBack()" class="btn btn-default btn-xs">← Back</button>
						</div>
						<?php echo '<h3 style="color:#900;">'.$this->session->flashdata('message2').'</h3>'; ?>
					</div>
...
<div class="portlet light">
	<div class="portlet-title">
		...
		<?php 
			$order_revision_review = $this->db->query("SELECT * FROM order_revision_review_csr WHERE order_id='$order_id' AND version='".$rev_row['version']."'")->row_array();
			if(!isset($order_revision_review['id'])){ 
			    $version = substr($rev_row['version'], -1); 
                if($version != 'a'){
                    $prev_version = --$version;
                    $v = 'V1'.$prev_version;
                    $prev_rev_chk = $this->db->query("SELECT `id` FROM rev_sold_jobs WHERE `order_id`= '$order_id' AND `version` = '$v' AND `rov_csr` = '".$this->session->userdata('sId')."'")->row_array(); 
                }else{
                    $prev_rev_chk = $this->db->query("SELECT `id` FROM rev_sold_jobs WHERE `order_id`= '$order_id' AND `rov_csr` = '".$this->session->userdata('sId')."'")->row_array(); 
                }   
                    if(isset($prev_rev_chk['id'])){
        ?>
			<div class="row">
				<div class="col-sm-6">
					<form action="<?php echo base_url().'index.php/new_csr/home/orderview_history/'.$hd.'/'.$order_id; ?>" method="post">
						<p><label>Did you make a mistake?</label></p>
						<label><input type="radio" class="form-control" value="yes" name="mistake" required>Yes</label>
						<label><input type="radio" class="form-control" value="no" name="mistake">No</label>
						<input type="hidden" name="version" value="<?php echo $rev_row['version']; ?>">
						<input type="hidden" name="rev_id" value="<?php echo $rev_row['id']; ?>">
						<p><button type="submit" name="submit_mistake">Submit</button></p>
					</form>
				</div>
			</div>
		<?php  }  } ?>
...
<!--			
			<?php 
			$order_revision_review = $this->db->query("SELECT * FROM order_revision_review_csr WHERE order_id='$order_id' AND version='V1'")->row_array();
			if(!isset($order_revision_review['id'])){ 
			    $version = substr($rev_row['version'], -1); 
                if($version != 'a'){
                    $prev_version = --$version;
