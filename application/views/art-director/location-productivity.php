...
{	
	...
	{
		$result = $this->db->query("SELECT * FROM `Designer_Production` WHERE `dlocation`='$location_id' AND `date` BETWEEN '$from' AND '$to' ;")->result_array();
	}else{
		$result = $this->db->get_where('Designer_Production',array('dlocation' => $location_id, 'date' => $ystday))->result_array();
	}
