...
<div class="page-container">
    ...
    <div class="page-content">
        ...
        <div class="navbar navbar-default" role="navigation">
								...
								<div class="navbar-header">
									...
									<a class="navbar-brand" href="javascript:;"><?php if(isset($form)) { $types = $this->db->get_where('help_desk', array('id'=>$form))->result_array(); ?>
									<?php echo $types[0]['name'];  }?> </a>
								</div>
	...
	foreach($newspaper as $row)
	{
			...
			{
				$cat_id = $this->db->query("SELECT * FROM `cat_result` WHERE `news_id`='$newspaper_id' AND (`date` BETWEEN '$from' AND '$to') ;")->result_array();
			}else{
				$cat_id = $this->db->query("SELECT * FROM `cat_result` WHERE `news_id`='$newspaper_id' AND (`date` BETWEEN '$ystday' AND '$today') ;")->result_array();		
		...
		//$pml = 0; $nml = 0;
		$tot_jobs = count($cat_id);
		
		foreach($cat_id as $row1){	
		
			//$width = $width + $row1['width'];
			//$height = $height + $row1['height'];
			$sq_inch = $sq_inch + ($row1['width'] * $row1['height']);
				if($row1['category'] == 'A')
				{
					$cat_a++; $tot_a++;
				}
...
?>
 ...
 <?php } ?>
			  <tr class="odd gradeX"> 
				...
				<!--
				<td><?php echo '<b>'.$tot_pml.'</b>'; ?></td> 
				<td><?php echo '<b>'.$tot_nml.'</b>'; ?></td>
				-->
