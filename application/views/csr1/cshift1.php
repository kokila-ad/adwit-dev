...
<script type="text/javascript">
	$(document).ready(function(e) {
        ...
        $('#help_desk').change(function(e) {
            window.location = "<?php echo base_url().index_page().'csr/home/cshift_verify/';?>" + $('#help_desk').val() ;
        });
...
<script type="text/javascript">	//all pending
	$(document).ready(function(e) {
        $('#display_type').change(function(e) {
            ...
            window.location = "<?php echo base_url().index_page().'csr/home/cshift_verify/'.$form.'/';?>" + $('#display_type').val() ;
        });
    });
</script>
  ...
  <div style="float: left;">
    ...
    <div class="block">
      <div class="navbar navbar-inner block-header">
		...
		<div style="float: right;">
		<button class="btn btn-danger btn-mini" >Count : <?php echo count($orders); ?></button>
		<button class="btn btn-success btn-mini" onclick="tableToExcel('testTable', 'W3C Example Table')" >Export to Excel</button>
		</div>
	  </div>
...
<!-- category -->		<?php if(($cat_result && $cat_result[0]['cancel']!='0') || $row1['cancel']!='0'){ 
							...
							<td><a href="javascript:;" onclick="window.location ='<?php echo base_url().index_page().'csr/home/cshift_category_v2/'.$form.'/'.$row1['id'];?>'" style="cursor:pointer; text-decoration: none;"><button class="btn btn-primary btn-mini" >category</button></a><?php if($row1['crequest']=='1'){echo" Cancel Request Sent";}elseif($row1['question']=='1'){echo" Question Sent";}elseif($row1['question']=='2'){echo" Question Answered";} ?></td>
						 <?php } ?>
...
<!-- Category ends --> 		  
<?php }elseif($display_type=='inproduction'){ ?>
<!-- Inproduction starts --> 
		<table cellpadding="0" cellspacing="0" border="0" class="table table-striped table-bordered" id="example">
            <thead>
            <tr>
...
<!-- design-->         <?php if($cat_result && $cat_result[0]['slug']!='none'){ 
					...
					<td>
						<div class="btn-group">
						<a href="javascript:;" onclick="window.location ='<?php echo base_url().index_page().'csr/home/QC/'.$form.'/'.$row1['id'];?>'" style="cursor:pointer; text-decoration: none;"><button class="btn btn-danger btn-mini" >Question Sent</button></a>
...
<!-- QA ends -->
<?php }elseif($display_type=='upload'){ ?>
<!-- upload starts -->
		<table cellpadding="0" cellspacing="0" border="0" class="table table-striped table-bordered" id="example">
            <thead>
            <tr>
...
<!-- upload -->		  <td>
						<?php if($row1['crequest']!='0'){ echo "Cancel Request Sent" ; }elseif(($cat_result && $cat_result[0]['cancel']!='0') || $row1['cancel']!='0'){ echo "Cancelled" ; }elseif($job_status){
...
<!-- upload ends -->
<?php }elseif($display_type=='pending'){ ?>
<!-- New Pending starts -->	
	<table cellpadding="0" cellspacing="0" border="0" class="table table-striped table-bordered" id="example">
            <thead>
            <tr>
...
<!-- Status -->			<td><?php echo $orderstatus[0]['name']; ?></td>

<!-- Q&A -->			<td> 
							<?php if($row1['question']!='0'){echo"Q";}else{echo"";} ?>
						</td>

<!-- View -->			<td> <div class="btn-group">
