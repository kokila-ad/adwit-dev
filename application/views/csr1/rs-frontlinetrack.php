...
<script type="text/javascript">
	$(document).ready(function(e) {
        $('#help_desk').change(function(e) {
            window.location = "<?php echo base_url().index_page().'csr/home/rs_frontlinetrack_all/';?>" + $('#help_desk').val() ;
        });
    });
</script>
...
<script type="text/javascript">
	$(document).ready(function(e) {
        $('#display_type').change(function(e) {
            ...
            window.location = "<?php echo base_url().index_page().'csr/home/rs_frontlinetrack_';?>" + $('#display_type').val() + "<?php echo '/'.$form; ?>" ;
        });
    });
</script>
...
$count = '0'; $cat_time = '0'; $timer = '0';
	...
	if($display_type=='pending')
	{
		if(isset($date)){
			...
			$jobs = $this->db->get_where('status_check',array('help_desk' => $form, 'category !=' =>'v1', 'date'=> $date, 'out_time'=> '00:00:00', 'job_status'=>'1'))->result_array();
			//$count = count($jobs)+1;
		}else{
			$this->db->order_by("id", "asc");
			$jobs = $this->db->get_where('status_check',array('help_desk' => $form, 'category !=' =>'v1', 'date'=> $today, 'out_time'=> '00:00:00', 'job_status'=>'1'))->result_array();
			//$count = count($jobs)+1;
		}
	}
	...
	if($display_type=='all')
	{
		if(isset($date)){
			$this->db->order_by("id", "asc");
			$jobs = $this->db->get_where('status_check',array('help_desk' => $form, 'category !=' =>'v1', 'date'=> $date, 'job_status'=>'1'))->result_array();
			 
		}else{
			$this->db->order_by("id", "asc");
			$jobs = $this->db->get_where('status_check',array('help_desk' => $form, 'category !=' =>'v1', 'date'=> $today, 'job_status'=>'1'))->result_array();
...
?>										
											<tr class="odd gradeX">
											  <td><?php echo $count ; ?></td>
												...
												<td><?php if($row['category']=='new'){ echo $row['in_time'];} ?></td>
												<td><?php if($row['category']=='pickup'){ echo $row['in_time'];} ?></td>
												<td><?php if($row['category']=='revision'){ echo $row['in_time'];} ?></td>
												<td><?php if($row['category']=='fastrack'){ echo $row['in_time'];} ?></td>
												<td><?php if($row['category']=='sold'){ echo $row['in_time'];} ?></td>
                                                <td><?php if($time_left < $cat_time && $row['out_time']=='00:00:00')
														 { 
												...
												<td>
												<?php if($row['out_time']!='00:00:00'){ echo $row['out_time'] ; }else{ ?>
												<form method="post">
...
<?php if($row['time_taken']!='0')
		  { 
			...
			echo $final_hours . ":" . $final_minutes . ":" . $final_seconds; 
												...
												<!--
												<?php if($row['job_status']=='1'){ ?>
												<td><?php if($row['out_time']=='00:00:00'){  ?>
												<form method="post">
