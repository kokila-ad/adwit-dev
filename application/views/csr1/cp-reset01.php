...
<div id="slug-view">
	...
	<?php 
	if(isset($cp_tool)){ 
		foreach($cp_tool as $row)
		{
			$csr = $this->db->get_where('csr',array('id' => $row['csr']))->result_array();
			echo $csr[0]['name']."<br/>";
			$errors = $this->db->get_where('cp_error_result',array('cp_result_id' => $row['id']))->result_array();
			foreach($errors as $row1)
			{
				$ename = $this->db->get_where('dp_error',array('id' => $row1['error_name']))->result_array();
				echo "<b>".$ename[0]['name']."</b>"."<br/>";
			}echo "<br/>";
		}
	 } ?>
...
<div id="dp-view">
	...
	<form name="form1" action="<?php echo base_url().index_page().'csr/home/cp_end01';?>" method="post" >
        	<h3 style="padding:0; padding-bottom: 5px; margin: 0; font-size: 18px;"> Issues&nbsp;<span style="font-size: 12px; color: #999; font-weight: normal;">(<a href="#designErrors" data-toggle="modal" style="text-decoration: none; color: #999; outline: none;">Help</a>)</span></h3>
	<?php
		$dp_error= $this->db->get_where('dp_error',array('group' => '0', 'status'=>'0'))->result_array();
