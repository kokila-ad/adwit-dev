...
<?php 
			...
			if(isset($from) && isset($to)){
				$orders = $this->db->query("SELECT * FROM `orders` WHERE `help_desk`='".$form."' AND `status`='2' AND `cancel`!='1' AND `crequest`!='1' AND (`created_on` BETWEEN '$from' AND '$to') ;")->result_array();
			}else{
				$orders = $this->db->query("SELECT * FROM `orders` WHERE `help_desk`='".$form."' AND `status`='2' AND `cancel`!='1' AND `crequest`!='1' AND (`created_on` BETWEEN '$ystday' AND '$today') ;")->result_array();		
			}
