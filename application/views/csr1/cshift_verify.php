        ...
        <div class="span12">
          <table cellpadding="0" cellspacing="0" border="0" class="table table-striped table-bordered" id="example">
            ...
            <tr>
                ...
                <?php if($form == '11'||$form == '10'){ ?><th style="text-align: center;">Priority</th><?php } ?>
...
<?php 
			...
			{
				$orders = $this->db->query("SELECT * FROM `orders` WHERE `help_desk`='".$form."' AND `status`='1' AND `cancel`='0' AND `crequest`!='1' AND (`created_on` BETWEEN '$from' AND '$to') ;")->result_array();
			}else{
				$orders = $this->db->query("SELECT * FROM `orders` WHERE `help_desk`='".$form."' AND `status`='1' AND `cancel`='0' AND `crequest`!='1' AND (`created_on` BETWEEN '$ystday' AND '$today') ;")->result_array();		
			}
...
<!--         <td>Pending</td>
   ...
   <?php if(($cat_result && $cat_result[0]['cancel']!='0') || $row1['cancel']!='0')
						...
						<?php if($form == '11'||$form == '10'){ 
        if($row1['priority']=='0000-00-00'){
         $date1=date_create(date('Y-m-d'));
         $date2=date_create(date('Y-m-d'));
         $publish_date = date_diff($date1,$date2);
        }else{ 
         //if(strtotime($row1['priority']) < strtotime(date('Y-m-d'))){
          $date1=date_create($row1['priority']);
          $date2=date_create(date('Y-m-d'));
          $publish_date = date_diff($date1,$date2); 
         //}else{ $publish_date = '0000-00-00'; }
        } 
        echo "<td>".$publish_date->format("%a days")."</td>"; 
       } 
      ?>
	</tr>
   <?php $i++; } ?>
