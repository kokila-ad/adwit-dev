...
<?php 
			...
			{
				$orders = $this->db->query("SELECT * FROM `orders` WHERE `help_desk`='".$form."' AND `status`='4' AND `cancel`!='1' AND `crequest`!='1' AND (`created_on` BETWEEN '$from' AND '$to') ;")->result_array();
			}else{
				$orders = $this->db->query("SELECT * FROM `orders` WHERE `help_desk`='".$form."' AND `status`='4' AND `cancel`!='1' AND `crequest`!='1' AND (`created_on` BETWEEN '$ystday' AND '$today') ;")->result_array();		
			}
