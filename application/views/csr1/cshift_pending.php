...
<?php if(isset($form)){?>
<meta http-equiv="refresh" content="<?php echo "60"; ?>;URL='<?php echo base_url().index_page().'csr/home/cshift/'.$form.'/pending'; ?>'">
<?php } ?>
 ...
 ?>
  ...
  <div style="float: right;">
		...
		<select id="display_type" name="display_type" style="width:80px; height:20px;" >
			...
			<option value="new_pending" <?php echo ($display_type=='new_pending' ? 'selected="selected"' : ''); ?> >new pending</option>
...
<?php 
			...
			{
				$orders = $this->db->query("SELECT * FROM `orders` WHERE `help_desk`='".$form."' AND (`created_on` BETWEEN '$from' AND '$to') ;")->result_array();
			}else{
				$orders = $this->db->query("SELECT * FROM `orders` WHERE `help_desk`='".$form."' AND (`created_on` BETWEEN '$ystday' AND '$today') ;")->result_array();		
			}
