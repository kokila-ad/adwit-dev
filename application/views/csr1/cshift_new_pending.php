...
<?php if(isset($form)){?>
<meta http-equiv="refresh" content="<?php echo "60"; ?>;URL='<?php echo base_url().index_page().'csr/home/cshift/'.$form.'/new_pending'; ?>'">
<?php } ?>
...
<?php 
	...
	{
		$orders = $this->db->query("SELECT * FROM `orders` WHERE `help_desk`='".$form."' AND `cancel`='0' AND `crequest`!='1' AND (`status` BETWEEN '0' AND '4') AND (`created_on` BETWEEN '$from' AND '$to') ;")->result_array();
	}else{
		$orders = $this->db->query("SELECT * FROM `orders` WHERE `help_desk`='".$form."' AND `cancel`='0' AND `crequest`!='1' AND (`status` BETWEEN '0' AND '4') AND (`created_on` BETWEEN '$ystday' AND '$today') ;")->result_array();		
	}
		...
		foreach($orders as $row1)
		{
			...
			$orderstatus = $this->db->get_where('order_status',array('id'=>$row1['status']))->result_array();
...
<!-- Status -->			<?php if($row1['status']=='1') { ?><td <?php if($row1['question']=='1') { echo "style='background: red;'";} if($row1['question']=='2') { echo "style='background:green;'";} ?>><a href="javascript:;" onclick="window.location ='<?php echo base_url().index_page().'csr/home/cshift_category_v2/'.$form.'/'.$row1['id'];?>'" style="cursor:pointer; text-decoration: none;" ><button class="btn btn-primary btn-mini" >category</button></a></td><?php } ?>
<!-- Status -->			<?php if($row1['status']=='2') { ?><td <?php if($row1['question']=='1') { echo "style='background: red;'";} if($row1['question']=='2') { echo "style='background:green;'";} ?>><a href="javascript:;" onclick="window.location ='<?php echo base_url().index_page().'csr/home/QC/'.$form.'/'.$row1['id'];?>'" style="cursor:pointer; text-decoration: none;"><button class="btn btn-danger btn-mini" >View</button></a></td><?php } ?>
<!-- Status -->			<?php if($row1['status']=='3') { ?><td <?php if($row1['question']=='1') { echo "style='background: red;'";} if($row1['question']=='2') { echo "style='background:green;'";} ?>><a href="javascript:;" onclick="window.location ='<?php echo base_url().index_page().'csr/home/cshift_cp_tool/'.$form.'/'.$row1['id'];?>'" style="cursor:pointer; text-decoration: none;"><button class="btn btn-primary btn-mini" >QA</button></a></td><?php } ?>
<!-- Status -->			<?php if($row1['status']=='4') { ?><td <?php if($row1['question']=='1') { echo "style='background: red;'";} if($row1['question']=='2') { echo "style='background:green;'";} ?>><a href="<?php echo base_url().index_page().'csr/home/pdf_upload/'.$row1['id'].'/'.$job_status[0]['id'];?>" onclick="javascript:void window.open('<?php echo base_url().index_page().'csr/home/pdf_upload/'.$row1['id'].'/'.$job_status[0]['id'];?>','1432728298066','width=800,height=650,toolbar=0,menubar=0,location=0,status=0,scrollbars=0,resizable=0,left=50,top=20');return false;"><button class="btn btn-success btn-mini">Upload</button></a></td><?php } ?>
...
-->
<!-- View 			<td>
							<a href="javascript:;" onclick="window.location ='<?php echo base_url().index_page().'csr/home/QC/'.$form.'/'.$row1['id'];?>'" style="cursor:pointer; text-decoration: none;"><button class="btn btn-danger btn-mini" >View</button></a>
						</td>
-->						
<!-- Category 		<?php if(($cat_result && $cat_result[0]['cancel']!='0') || $row1['cancel']!='0'){ 
							...
							<td><a href="javascript:;" onclick="window.location ='<?php echo base_url().index_page().'csr/home/cshift_category_v2/'.$row1['id'];?>'" style="cursor:pointer; text-decoration: none;"><button class="btn btn-primary btn-mini" >category</button></a></td>
						 <?php } ?>
-->						 
