						...
						<tbody>	
							...
							if(isset($orders_rev)){
								foreach($orders_rev as $row){
									...
									if($row['pdf_path']!='none'){ 
										...
										if(!file_exists($row['pdf_path'])){ $rev_path = $row['pdf_path'].'/'.$row['pdf_file']; }
									}
									if(isset($sourcefilepath) && ($row['status']=='5' || $row['status']=='9') && $row['source_file']!='none'){
							...
							?>
														<tr>
															...
															<td><?php if($row['question']=='1'){ ?>
																<a href="javascript:;" onclick="window.location = '<?php echo base_url().index_page().'new_client/home/rev_ad_answer/'.$row['id'];?>'" style="cursor:pointer; text-decoration: none;">
															...
															<td><?php if($row['pdf_path']!='none' && file_exists($rev_path)){ ?>	
																<a href="<?php echo base_url().$rev_path ?>" data-toggle="tooltip" title="<?php if(isset($note_rev['id'])){ echo $note_rev['note']; }else{ echo"PDF"; } ?>" data-placement="top" target="_Blank" data-toggle="tooltip" title="PDF" ><img src="<?php echo base_url(); ?>assets/new_client/img/pdf.png"></a>
																<?php  }else{ echo ''; } ?>
															</td>
															
															<!--Source File Download -->
															<td>
																<?php if(isset($ftp_source_path)) { ?>
																<a href="<?php echo $ftp_source_path; ?>" target="_blank"><button type="" name="SourceDownload" class="btn btn-blue btn-sm">
																Source Package</button></a>
																<?php } else { 
																	 if($publication['enable_source']=='1' && $row['source_file']!='none' && isset($source_file_path) && file_exists($source_file_path)){ ?>
						   ...
						   <tr>
							   ...
							   <td><?php if($order[0]['pdf'] != 'none' && file_exists($pdf_path)){ ?>
										<a href="<?php echo base_url().$pdf_path ?>" data-toggle="tooltip" title="<?php if(isset($note['id'])){ echo $note['note']; }else{ echo"PDF"; } ?>" data-placement="top" target="_Blank" data-toggle="tooltip" title="PDF" ><img src="<?php echo base_url(); ?>assets/new_client/img/pdf.png"></a>
									<?php }else{ echo ''; }?>
							   </td>
							   <!--Source File Download --> 
							   <td>
								   <?php if(isset($ftp_source_path)) { ?>
										...
										Source Package</button></a>
										<?php }  else {
										if($publication['enable_source']=='1' && isset($sourcefilepath)){ 
