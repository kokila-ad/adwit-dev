...
$(document).ready(function(){
    ...
    $('#date_range').change(function() {
        if($(this).val() == 'custom'){
            ...
            $('#custom').hide();
    	    window.location = "<?php echo base_url().index_page().'new_client/home/report_advertiser/';?>" + $('#advertiser').val() + "?date_range=" + $('#date_range').val() ;
        }
    });
    
    $('#advertiser').change(function() {
        window.location = "<?php echo base_url().index_page().'new_client/home/report_advertiser/';?>" + $('#advertiser').val() ;
    });
});
</script>
    ...
    <div class="container">
        <div class="row margin-top-15 margin-bottom-15">
			<div class="col-md-4 col-md-offset-1" >
                ...
                <label>Choose an Advertiser from this list:
                    <input list="browsers" class="form-control" name="advertiser" id="advertiser" value="<?php if(isset($advertiser) && $advertiser != '') echo $advertiser; ?>"/>
                </label>
                <datalist id="browsers" >
                    <?php foreach($advertiser_list as $row){ ?>
                        <option value="<?php echo $row['advertiser_name']; ?>">
                    <?php } ?>
                </datalist>
		  
			</div>
		</div>
	<?php if(isset($advertiser) && $advertiser != ''){ ?>	
		<div class="row margin-top-15 margin-bottom-15">
			<div class="col-md-4 col-md-offset-1" >
		...
		<div class="row margin-top-15" id="custom" style="display:none">
			  <form method="GET" action="<?php echo base_url().index_page().'new_client/home/report_advertiser/'.$advertiser; ?>" autocomplete="off" >
