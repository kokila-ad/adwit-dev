...
<!-- Approve popup modal css code START--> 
<style>
...
<!-- Approve popup modal css code END-->
				...
				<table class="table table-striped table-bordered table-hover" id="example1">
					...
					<thead>
						<tr>
							...
							<th class="width-120">Page Number</th>
							<?php }else{ ?>
							<th class="width-120">Unique Job ID</th>
							<?php } ?>
							
							<?php if($client[0]['page']=='1'){ ?>
							<th class="width-120">Section</th>
							<?php }else{ ?>
							<th class="width-120">Advertiser</th>
							<?php } ?>
							
							<?php if($client[0]['team_orders']=='1'){ ?>
							<th class="width-90">Ad Rep</th>
							<?php } ?>
							
							<!-- waukesha freeman -->
							<?php 
							    if($publication[0]['id']=='580'){ 
							        echo '<th class="width-90">Keyword</th>'; 
							        echo '<th class="width-90">Account No.</th>'; 
							    } 
							?>
							
							<th class="width-90">Publish Date</th>
							
							<?php if($publication[0]['id']=='538'){ ?>
							<th class="width-90">Publication</th>
							<th class="width-90">Section</th>
							<th class="width-90">Deadline</th>
							<?php } ?>
...
<!-- Status -->
					<td class="width-90"><!-- Question sent for Answer -->
						<?php 
						    echo $order_status; 
						    if($row['question']=='1'){ 
						?>
						    <a href="javascript:;" onclick="window.location = '<?php echo base_url().index_page().'new_client/home/new_ad_answer/'.$row['id'];?>'" style="cursor:pointer; text-decoration: none;">
						        <button class="btn red-thunderbird btn-grey btn-xs">Question</button>
						    </a>
						<?php }elseif($orders_rev && $orders_rev['question']=='1'){ ?>
						    <a href="javascript:;" onclick="window.location = '<?php echo base_url().index_page().'new_client/home/rev_ad_answer/'.$orders_rev['id'];?>'" style="cursor:pointer; text-decoration: none;">
						        <button class="btn btn-block btn-xs padding-5 btn-grey" title="<?php echo $orders_rev['question']; ?>">Question</button>
						    </a>
						<?php 
						      } 
						        
						        if($row['crequest']!='0'){ echo'<p><small>(Cancel Requested)</small></p>'; } 
						?>
						<!-- skip and download -->
        				<?php 
		...
		<!--Resubmit -->
			...
			<!--cancel button -->
							...
							<!--	<form method="post" action="<?php echo base_url().index_page().'new_client/home/order_cancel/'.$row['id'];?>">
									...
									<button type="button" name="remove" id="remove" class="btn btn-block btn-xs padding-5 btn-grey" onclick="cancelOrderModal(<?php echo $row['id'];?>)">Cancel</button>
									<div style="display:none" id="cancel_order_div"></div>
							...
							<!--<td title="Job Cancel">
								...
								<button class="btn btn-block btn-xs padding-5 btn-grey cursor-pointer padding-left-5" type="button" data-toggle="dropdown" id="view">Cancel</button>
...
<!-- Actions -->							
		...
		<!-- Approve Unapprove -->
							...
							<?php }else{ ?>
								<td title="Job Approval">
								<?php if($publication[0]['id']=='190'){  //VIDN ?>  
									<a href="<?php echo base_url().index_page().'new_client/home/approval/'.$row['id'] ;?>">
									<button class="btn btn-block btn-xs padding-5 btn-success">Approve</button>
									</a>
								<?php }elseif(in_array($publication[0]['id'], $pub_permit)){ //Lethbridge herald, Taber times popup modal ?>
		...
		<!-- cancel -->
			...
			<?php if(empty($pre_next)) {?>
				 <div class="row">	
					<form method="get" action="<?php echo base_url().index_page().'new_client/home/dashboard/'.$count;?>">
				
					<div class="col-xs-1 col-md-offset-10">
					<?php if($count != '0') { ?>
						<button name="previous" type="submit" class="btn btn-sm btn-dark">Previous</button>
					<?php } ?>
					</div>	
					<div class="col-xs-1 ">
						<button name="next" type="submit" class="btn btn-sm btn-dark">Next</button>
					</div>
					</form>
				</div>
				<?php } ?>
...
<div class="modal" id="orderCancelModal" tabindex="-1"  role="dialog" aria-labelledby="confirmationModalTitle" >
 <div class="modal-dialog modal-dialog-centered" role="document" style="width:700px;">
    <div class="modal-content" style="width:85%">
      <div class="modal-header" style="border-bottom: 0 !important;background-color:#333 !important;">
        <h5 class="modal-title portlet-title margin-top-10" id="orderCancelModal" style="margin-top: 0px !important; padding:10px !important;"><center><b>Alert</b></center></h5>
      </div>
      <div class="modal-body" style="padding: 15px 16px;">
         <center><b>Are you sure you want to cancel ?</b></center>
      </div>
...
<script>
    ...
    function confirm(){
        ...
        var submitInput = '<input type="text" name="remove">';
        $('#cancel_order_div').append(submitInput);
         form.action = base_url + index_page + 'new_client/home/order_cancel/' + order_id;
        // Close the modal
